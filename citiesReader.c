...
int calculer_distance( int villeD, int villeA, ListOfCities * cities)
{
  float distance;
  float valeur;
  valeur = 3.14159265/180;
  
  float lon_D = cities->lon[villeD];
  float lat_D = cities->lat[villeD];
  
  float lon_A = cities->lon[villeA];
  float lat_A = cities->lat[villeA];
  
  distance= 6371 * acos(sin(lat_D*valeur) * sin(lat_A*valeur) + cos((lon_D*valeur)-(lon_A*valeur)) * cos(lat_D*valeur) * cos(lat_A*valeur));
  return distance; 
} 